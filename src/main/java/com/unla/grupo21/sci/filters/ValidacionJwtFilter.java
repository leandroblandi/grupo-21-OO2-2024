...
/**
 * Esta clase, intercepta todas las peticiones y extrae el Token de
 * autenticacion, lo valida, y permite o denega la petición, segun sea
 */
public class ValidacionJwtFilter extends BasicAuthenticationFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
		...
		/**
		 * Aquí es donde se obtienen los claims, y se verifica el token, una vez
		 * realizada la validación, se sigue la cadena de filtros
		 */
		try {
			Claims claims = Jwts.parser().verifyWith(TokenJwtConfiguration.SECRET_KEY).build().parseSignedClaims(token)
					.getPayload();
